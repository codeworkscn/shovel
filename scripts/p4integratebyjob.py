...
def execute(jobName):
    ...
    if not changelistsToMerge:
        print("skip integrate due find no changelist by job [%s]" % jobName)
        return
    ...
    description = "Merging for job %s:\n\nfrom %s\nto %s" % (
        jobName, source, target)
    saveToChangeId = p4Client.p4_new_changelist(description)
    print("saveToChangeId=%s" % saveToChangeId)

    p4Client.p4_integrate_by_changelists(
        source, target, changelistsToMerge, saveToChangeId)
    print("integrate done, jobName=%s, saveToChangeId=%s" %
          (jobName, saveToChangeId))
    ...
    print(
        "please check pending changelist [%s], add comment, then submit your changes." % saveToChangeId)
